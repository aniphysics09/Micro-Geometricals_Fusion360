...
def run(_context):
    ...
    try:
        ...
        # ========== Add 500 Small Random Cylinders ==========
        surface_z = 5

        for _ in range(500):
            dia = random.uniform(0.2, 0.5)
            ...
            sketch_cyl.sketchCurves.sketchCircles.addByCenterRadius(adsk.core.Point3D.create(x, y, 0), r)
            prof = sketch_cyl.profiles.item(0)

            ext_input_cyl = extrudes.createInput(prof, adsk.fusion.FeatureOperations.NewBodyFeatureOperation)
            ext_input_cyl.setDistanceExtent(False, adsk.core.ValueInput.createByReal(h))
            ext_cyl = extrudes.add(ext_input_cyl)

            # Move the new cylinder up to Z=5 mm
            cyl_body = ext_cyl.bodies.item(0)
            moveFeats = rootComp.features.moveFeatures
            transform = adsk.core.Matrix3D.create()
            transform.translation = adsk.core.Vector3D.create(0, 0, surface_z)
            move_objs = adsk.core.ObjectCollection.create()
            move_objs.add(cyl_body)
            moveInput = moveFeats.createInput(move_objs, transform)
        ...
        ui.messageBox("Model created:\nCuboid base + wall + 500 random standing cylinders + 3 corner cubes.")
